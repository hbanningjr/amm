...
const Deposit = () => {
  ...
  const amountHandler = async (e) => {
    ...
    if (e.target.id === "token1") {
      setToken1Amount(e.target.value); // user typed - keep as is
      ...
      setToken2Amount(parseFloat(_token2Amount).toFixed(4)); // <-- round this
    } else {
      setToken2Amount(e.target.value); // user typed - keep as is
      ...
      setToken1Amount(parseFloat(_token1Amount).toFixed(4)); // <-- round this
    }
  };
          ...
          >
            <Row>
              <Form.Text className='text-end my-2' muted>
                Balance: {Number(balances[0]).toFixed(4)}
              </Form.Text>
              <InputGroup>
                <Form.Control
                  type='number'
            ...
            <Row className='my-3'>
              <Form.Text className='text-end my-2' muted>
                Balance: {Number(balances[0]).toFixed(4)}
                <Form.Text className='text-end my-2' muted></Form.Text>
              </Form.Text>
              <InputGroup>
                <Form.Control
                  type='number'
